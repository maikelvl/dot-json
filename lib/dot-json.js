...
module.exports = function(file) {
	...
	this._readJsonSync = function(log) {
		...
		var content = fs.readFileSync(_this._file);
		_this._object = JSON.parse(content);
		_this._exec_read_queue();
		_this._wait_for_read = false;
	}
	...
	this.get = function(key_path, callback) {
		...
		// 	throw new Error('Please pass a callback function as first or second argument');
		// }
		_this._readJsonSync();
		var object;
	...
	this.save = function(indent, callback) {
		...
		const dump = () => JSON.stringify(_this._object, null, indent) + '\n'
